...
# This script installs kubeflex binaries and plugins to a folder of choice
#
# Arguments:
# [--version release] set a specific kubeflex release version, default: latest
...
# [--ensure-folder name] sets the installation folder, default: $PWD/kubeflex
...
if [ $verbose == "true" ]; then
    echo "Downloading kubeflex $version $os/$arch..."
    curl -SL -o kubeflex.tar.gz "https://github.com/kubestellar/kubeflex/releases/download/${version}/kubeflex_${version//v}_${os}_${arch}.tar.gz"
else
    curl -sSL -o kubeflex.tar.gz "https://github.com/kubestellar/kubeflex/releases/download/${version}/kubeflex_${version//v}_${os}_${arch}.tar.gz"
fi
...
if [[ ! ":$PATH:" == *":$bin_folder:"* ]]; then
    echo "Add kubeflex folder to the path: export PATH="\$PATH:$bin_folder""
fi

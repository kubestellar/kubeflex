...
func installAndPatchNginxIngress() error {
	...
	defer os.Remove(patchFile.Name()) // clean up
...
func main() {
	log.Println("Creating kind instance...")
	err := createKindInstance("kubeflex")
	if err != nil {
		log.Fatalf("Error creating kind instance: %v\n", err)
	}
	log.Println("Kind instance created successfully.")

	log.Println("Installing and patching nginx ingress...")
	err = installAndPatchNginxIngress()
	if err != nil {
		log.Fatalf("Error installing and patching nginx ingress: %v\n", err)
	}
	log.Println("Nginx ingress installed and patched successfully.")
}

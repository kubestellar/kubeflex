...
type ControlPlaneLister interface {
	...
	List(selector labels.Selector) (ret []*tenancyv1alpha1.ControlPlane, err error)
	// Get retrieves the ControlPlane from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*tenancyv1alpha1.ControlPlane, error)
	ControlPlaneListerExpansion
}

// controlPlaneLister implements the ControlPlaneLister interface.
type controlPlaneLister struct {
	listers.ResourceIndexer[*tenancyv1alpha1.ControlPlane]
}
...
func NewControlPlaneLister(indexer cache.Indexer) ControlPlaneLister {
	return &controlPlaneLister{listers.New[*tenancyv1alpha1.ControlPlane](indexer, tenancyv1alpha1.Resource("controlplane"))}
}
